...
describe "/schedules/show.html.haml" do
  it "should show one schedule with its name, date, description and items" do
    assign(:schedule,
      stub("schedule4", :name => "First Schedule",
                        :created_at => 3.days.ago,
                        :description => "this is my first schedule, its just a test",
                        :items => [stub("item1", :level => 0, :text => "400m"),
                                  stub("item2", :level => 1, :text => "2000m"),
                                  stub("item3", :level => 1, :text => "200m")]))
